import {
	getConfiguration, disableLogs, formatMoney as importedFormatMoney,
	formatDuration, scanAllServers, log
} from './helpers.js'
	...
	while (true) {
		...
		if (options['hack-percent'] == -1) {
			...
			if (hack_percent <= 0 || hack_percent >= 1) {
				...
				log(ns, "hack-percent out of range (0-100)");
				return;
			}
		}
		...
		if (!options['silent'])
			ns.tprint("Best server: ", best_server.hostname, " with ", formatMoney(best_server.gainRate), " /RS");
		...
		for (const server of server_eval)
			serverListByGain += `\n ${order++} ${server.hostname}, with ${formatMoney(server.gainRate)} /RS while stealing ` +
				`${(server.estHackPercent * 100).toPrecision(3)}% (@ ${server.requiredHackingSkill})`;
		...
		if (!options['silent'])
			ns.tprint("Best exp server: ", best_exp_server.hostname, " with ", best_exp_server.expRate, " exp /RS");
		order = 1;
		...
		for (let i = 0; i < Math.min(5, server_eval.length); i++)
			serverListByExp += `\n ${order++} ${server_eval[i].hostname}, with ${server_eval[i].expRate.toPrecision(3)} exp /RS`;
		...
		*/
		await ns.sleep(2 * 1000); //update every 2 sec
	}
