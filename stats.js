...
async function getHudData(ns, bitNode, dictSourceFiles, options) {
	...
	hudData.push(["SctInEx", true, formatMoney(totalScriptInc[0], 3, 2) + '|' + formatNumberShort(totalScriptExp, 3, 2) + '/s', "Total 'instantaneous' income/exp per second being earned across all scripts running on all servers."]);
	//hudData.push(["SctExp", true, formatNumberShort(totalScriptExp, 3, 2) + '/s', "Total 'instantaneous' hack experience per second being earned across all scripts running on all servers."]);
	...
	// Show gang income and territory
	{
		...
		if ((2 in dictSourceFiles || 2 == bitNode) && gangInfo) {
			// Add Gang Income
			val1.push(true, formatMoney(gangInfo.moneyGainRate * 5, 3, 2) + '/s',
